...
export default function SignUpModal() {
     ...
     *****************************************************************************************************
    *****************************************************************************************************/
    const { signUp, signInWithGooglePopup, toggleModals, persistenceMode, togglePersistenceMode } = useContext(UserContext)
    ...
    *****************************************************************************************************/
    ...
    const [validationPolicies, setValidationPolicies] = useState(false)
    const [signUpEmailAddClass, setSignUpEmailAddClass] = useState("")
    const [signUpPwdAddClass, setSignUpPwdAddClass] = useState("")
    const [policiesInputBorderClasses, setpoliciesInputBorderClasses] = useState("border border-gray-300 dark:border-gray-500")
    ...
    **************************************************************************/
    const onSubmitForm = async (e) => {
        ...
        console.log("policies.current.checkd: ", policies.current.checked)

        //Vérification de l'acceptation des conditions d'utilisation
        if (!policies.current.checked) {
            //setValidationPolicies(validEmail.ruleLabel)

            setpoliciesInputBorderClasses("border-2 border-red-500 dark:border-red-500")
            return;
        }
        else {
            //setValidationMail("")
            setpoliciesInputBorderClasses("border border-gray-300 dark:border-gray-500")
        }

        //Verification de la syntaxe du mail
    ...
    *****************************************************************************************************/
    return (
        <>
            {/* backdrop */}
            <div id="inscription-modal" tabIndex="-1" aria-hidden="true" className="fixed top-[40px] z-50 w-full h-full backdrop-blur flex justify-center items-center" onClick={() => toggleModals("close")}>
            </div >
            ...
            <div className="mx-auto w-96 top-60 fixed inset-x-0 items-center z-[51] h-auto bg-white rounded-lg shadow dark:bg-gray-700 p-4">
                ...
                ${validationPolicies ? "hover:bg-slate-100" : "hover:bg-transparent text-slate-300"}`}>
                ...
                <form ref={formRef} className="space-y-6" onSubmit={onSubmitForm} noValidate>
                    ...
                    <button type="submit" className="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 disabled:bg-slate-300" disabled={validationPolicies ? false : true}>Créer votre compte</button>

                    <div className='border-b dark:border-gray-500 m-auto'></div>

                    <div className="w-full flex">
                        <input ref={policies} id="policies" type="checkbox" value="" className={`peer relative appearance-none shrink-0 w-4 h-4 border-2 border-blue-200 rounded-sm mt-1 bg-white focus:outline-none focus:ring-offset-0 focus:ring-1 focus:ring-blue-100 checked:bg-blue-500 checked:border-0 disabled:border-steel-400 disabled:bg-steel-400 ${policiesInputBorderClasses}`} onChange={() => setValidationPolicies(!validationPolicies)} required checked={validationPolicies} />
                        <svg className="absolute w-4 h-4 pointer-events-none hidden peer-checked:block stroke-white mt-1 outline-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
